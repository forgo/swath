...
/// samples. The scales can differ per axis (e.g. a target that overhangs the
...
/// this exactly; golden tests against oracle tiles with a real swath-edge
/// would fail on the alpha channel under any other default.
...
pub enum Resampling {
    ...
    /// categorical bands (e.g. quality masks), where interpolation would
    /// invent class codes.
    Nearest,
...
///
/// # Buffer design (feeds downstream pixel stages)
///
/// Values are **`f64`**, not the source dtype: every supported source dtype
/// is exactly representable in `f64`, bilinear results are intrinsically
/// fractional, and downstream stages (band math, rescale, colormap)
/// compute in floating point anyway. Quantizing back to the source dtype
/// here would bake rounding error into the pipeline before band math runs.
/// The validity mask is a separate `Vec<bool>` rather than a nodata
/// sentinel so downstream stages never have to guess whether a value is
/// data — invalid pixels hold `0.0` and must be interpreted through
/// [`valid`](Self::valid) (an encode stage typically turns it into the
/// alpha channel).
...
/// dimensions, from which the GDAL-equivalent resampling geometry is
/// derived.
///
/// The grid comes from the [`SourceBuffer`] itself (never passed
/// separately), so overview reads warp correctly by construction: an
/// overview window carries the overview grid, and every coordinate here —
/// window offsets, kernel window, raster bounds — is in that grid's pixel
/// space. GDAL behaves identically when warping from an overview: the same
/// warp machinery runs against the overview dataset's grid.
///
...
/// through [`CoordTransform::transform_slice`] so implementations' bulk
/// paths are used; a row whose batch fails falls back to per-point
/// transforms with failing pixels marked invalid.
///
/// The source window must cover the resampling support: bilinear needs the
/// target's source extent plus 1 pixel at scale ≥ 1, growing to
/// `ceil(1/scale) + 1` when the warp decimates (pass that as the `margin`
/// of [`crate::source_window`]).
///
/// # Errors
///
/// * [`WarpError::SourceShape`] — buffer length disagrees with its window.
/// * [`WarpError::NonInvertibleTransform`] — singular geotransform.
#[allow(
...
pub fn warp(
...
) -> Result<WarpedBuffer, WarpError> {
    let info = &source.grid;
    let win = source.window;
    let expected = win.width * win.height;
    let actual = source.samples.len() as u64;
    if expected != actual {
        return Err(WarpError::SourceShape { expected, actual });
    }
    ...
    let Some(extent) = source_extent(target, info, transform)? else {
        return Ok(out); // whole target outside the transform's domain
    };
    let Some(shape) = KernelShape::compute(
        ...
        return Ok(out); // whole target off the raster
    };
    ...
    for row in 0..target.height() {
        ...
        for (col, (&(sx, sy), &ok)) in points.iter().zip(point_ok.iter()).enumerate() {
            ...
            let Some((fcol, frow)) = src_transform.crs_to_pixel(sx, sy) else {
...
mod tests {
    ...
    use crate::source::{PixelWindow, SourceBuffer, SourceGrid};
    ...
    /// Raster metadata for a `w × h` grid: 1 m pixels, origin (0, 0) with y
    /// growing downward from 0 (`north_up` with `origin_y` = 0).
    fn grid_of(w: u64, h: u64) -> SourceGrid {
        SourceGrid {
            width: w,
            height: h,
    ...
    /// A window at offset (0, 0) covering the whole `w × h` raster.
    fn source(w: u64, h: u64, samples: &[f64], nodata: Option<f64>) -> SourceBuffer<'_> {
        SourceBuffer {
            grid: grid_of(w, h),
            window: PixelWindow {
                col_off: 0,
    ...
    fn identity_nearest_reproduces_the_source() {
        let samples = seq(16);
        let src = source(4, 4, &samples, None);
    ...
    fn bilinear_interpolates_between_centers() {
        ...
        let samples = [10.0, 30.0];
        let src = source(2, 1, &samples, None);
    ...
    fn nearest_rejects_nodata_and_out_of_window() {
        let samples = [7.0, -9999.0, 7.0, 7.0];
        let src = source(2, 2, &samples, Some(-9999.0));
    ...
    fn bilinear_renormalizes_around_nodata_but_propagate_invalidates() {
        ...
        let samples = [100.0, 100.0, 100.0, -9999.0];
        let src = source(2, 2, &samples, Some(-9999.0));
    ...
    fn out_of_domain_pixels_are_invalid_not_errors() {
        ...
        let src = source(4, 1, &samples, None);
        let target = TargetGrid::new(
            GridBounds {
                min_x: -2.0,
                min_y: -1.0,
    ...
    fn shape_mismatch_and_singular_transform_are_errors() {
        ...
        let samples = [0.0; 4];
        let mut src = source(2, 2, &samples, None);
    ...
    fn empty_source_window_yields_all_invalid() {
        let src = source(0, 0, &[], Some(-9999.0));
        let out = warp(&src, &Identity, &aligned_grid(2, 2), Resampling::Nearest).unwrap();
