...
//! # Examples
//!
//! A z11-like tile over a 30 m source (~505 full-res pixels per 256-px
//! axis, desired ratio ~1.97) with an embedded ×2 overview: GDAL's
//! calibrated 1.2 oversampling slack serves the overview.
//!
//! ```
//! use swath_planner::{Availability, BandWindow, Budget, CacheProbe, PlanChoice, plan};
//!
//! let availability = Availability::new(
//!     CacheProbe::Miss,
//!     256,
//!     vec![BandWindow::new(505.0, 505.0, 2, vec![2])],
//! );
//! let p = plan(&Budget::default(), &availability);
//! assert_eq!(p.strategy, PlanChoice::Overview { factor: 2 });
//! // Every candidate is recorded, chosen or not — cache_hit, overview, live.
//! assert_eq!(p.considered.len(), 3);
//! ```
//!
//! A budget ceiling refuses an unbounded full-resolution read instead of
//! performing it:
//!
//! ```
//! use swath_planner::{Availability, BandWindow, Budget, CacheProbe, PlanChoice, plan};
...
//!     ..Budget::default()
//! };
//! // No overviews, no cache: only live could serve — and it estimates
//! // 505 × 505 × 2 bytes ≈ 512 KB, over the ceiling.
//! let availability = Availability::new(
...
//! assert!(matches!(p.strategy, PlanChoice::Refuse { limit: 100_000, .. }));
//! ```
...
/// model and overview rule need, all of it already computed by the
/// caller before any pixel read (describe metadata + the tile-boundary
/// extent).
...
pub struct BandWindow {
    ...
    /// (pre-clip).
    pub rows: f64,
    /// Bytes per sample of the band's data type.
    pub bytes_per_sample: u64,
    /// Decimation factors of the asset's embedded overviews, as the
    /// source's metadata reports them.
...
/// A candidate strategy as the plan trace names it — the executed
/// strategy's shape minus execution details (a cache-hit candidate has
/// no key until it is served).
///
/// Wire form mirrors Swath's Trace `Strategy` vocabulary: `"cache_hit"`,
/// `{"overview": {"factor": 2}}`, `"live"`.
...
/// given `availability`, recording every candidate (crate docs; spec
