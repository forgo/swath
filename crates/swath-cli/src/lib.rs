...
//! The `swath` binary's library (ARCHITECTURE.md §12, issue #29; a lib
//! target since #353 so its wiring is reachable from tests and tools): the
//! single self-contained deployable. `swath serve` wires the concrete Phase-1
...
/// panicked.
pub fn report<E: std::fmt::Display>(result: Result<(), E>) -> ExitCode {
    match result {
        Ok(()) => ExitCode::SUCCESS,
        Err(err) => {
            tracing::error!("{err}");
