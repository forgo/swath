...
domain → STAC → domain is the identity. For **Items** the reverse now holds too: every property
Swath does not model rides in `Granule.properties` verbatim (ADR 0029), so a foreign Item's
properties survive STAC → domain → STAC unchanged. Collections are still not promised: one some
other tool wrote may carry fields Swath does not model, and converting it either fails loudly
(missing swath-required fields) or normalizes it.
...
| `assets` | `BTreeMap<String, GranuleAsset>` | band name → asset (`href: AssetRef` + `kind`); the map the tiler's `bands` template resolves against. `kind` (#40) distinguishes a plain raster (COG, the default) from a **virtual-cube manifest** (`VirtualManifest` JSON generated by the ingest referencer, ADR 0006) so serving dispatches the right reader (#39) |
...
| `properties` | `BTreeMap<String, Value>` | every other STAC property, verbatim and **opaque** (ADR 0029): `eo:cloud_cover`, `platform`, `proj:epsg` and anything else the item carried. Swath preserves and serves them and interprets none of them. `datetime` and `swath:*` are excluded — they are domain fields already, and one authority per fact |
...
| `assets` | `assets` — `{ "<band>": { "href": "<uri>" } }`, plus `"swath:kind": "virtual_cube"` for non-raster assets (omitted for the raster default, so pre-#40 documents are byte-identical) | `href` and `swath:kind` read back (absent kind = raster; present-but-unknown = loud error); other asset keys ignored, so Swath's own documents round-trip exactly |
...
| `properties` | every remaining `properties.*` key | carried both ways verbatim (ADR 0029); projected keys (`datetime`, `swath:*`) are never duplicated into it |
