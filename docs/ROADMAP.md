...
| 15 | **Dataset extents derived from ingested granules** | `crates/swath-cli/src/config.rs` | A whole-world open-ended placeholder is honest for a dataset whose coverage is whatever granules arrive; maintaining real extents is bookkeeping without a consumer | Discovery (Records) or EDR needs real spatial/temporal extents |
